...
<?php 
$ppp = 6;
$args = array( 'post_type' => 'post', 'order_by' => 'date', 'order' => 'DESC', 'post_status' => 'publish' );
...
<section dkit-grid="col-12" class="Services_section Blog-exercices Chroniques">
    ...
    <div class="sujets">
	    <p data-categorie="">
		    <?php echo strip_p( get_field('titre_chronique') ); ?>
...
<section dkit-grid="col-12" class="Blog Blog-exercices">
    ...
    <div dkit-grid="col-12" class="types">
        <ul>
            <li class="types-items">
            	<a class="categories" data-categorie="" data-total="<?php echo $found_posts; ?>">Toutes les chroniques</a>
	            <?php 
	            $subcategories = array('child_of' => 10);
	            $categories = get_categories( $subcategories ); 
	            foreach ( $categories as $category ) {
	                printf( '<li class="types-items"><a class="categories" data-categorie="%1$s" data-total="%3$s">%2$s</a>',
    ...
    <div class="Blog_roll" data-grid="col-12">
        ...
        while( $blog_query->have_posts() ) : $blog_query->the_post(); ?>
		    ...
		    <?php get_template_part('item-chroniques') ?>
    ...
    <div dkit-grid="col-12" class="Afficher_plus">
        <a id="more_posts">Afficher plus de chroniques <span class="icon-uni60"></span></a>
    </div>
...
//  ); 
	...
	var post_type = 'post';
	var total = <?php echo $found_posts; ?>;
	var cat = '';
...
$('.categories').on('click', function() {
    ...
    $.post(
        ...
        if( total <= ppp ) {
			...
			$("#more_posts").css({"opacity": "1"}).html('Il n\'y a plus de chroniques à afficher');
			$(".Afficher_plus").addClass('no-more');
        } else {
			$("#more_posts").css({ "display": "block", "pointer-events": "auto", "opacity": "1" });
			$("#more_posts").html('Afficher plus de chroniques <span class="icon-uni60"></span>');
			$(".Afficher_plus").removeClass('no-more');
        }
...
<?php if( $found_posts <= $ppp ): ?>
	$("#more_posts").css({"opacity": "1"}).html('Il n\'y a pas d\'autres chroniques à afficher');
	$(".Afficher_plus").addClass('no-more');
<?php endif; ?>
</script>