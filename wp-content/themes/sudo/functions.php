...
function my_acf_init() {
	...
	acf_update_setting('google_api_key', 'AIzaSyAiEmHReC6V2j1s5bi5Unwye8KT1XP2RDk');
}
...
function wpdocs_theme_setup() {
    add_image_size( '500x425', 500, 425, false ); // OLD- Image de fond des articles carrés sous l'article horizontal -OLD
    add_image_size( '500x925', 500, 925, true ); // OLD- Image de fond de l'article à la verticale -OLD
    add_image_size( '1000x500', 1000, 500, true ); // OLD- Image de fond de l'article à l'horizontale -OLD
    
    add_image_size( '600x600', 600, 600, true ); // Image de fond des articles carrés sous l'article horizontal
    add_image_size( '600x950', 600, 950, true ); // Image de fond de l'article à la verticale
    add_image_size( '950x600', 950, 600, true ); // Image de fond de l'article à l'horizontale
}
...
//Enlève les balises p et remplace celle qui sont à l'intérieur par DEUX <br>
...
//Enlève les balises p et remplace celle qui sont à l'intérieur par UN <br>
function strip_p_single( $string ) {
  $string = str_replace("<p>", "", $string);
...
$output = "<select name='select_services' id='select_services' onchange='document.getElementById(\"select_services\").value=this.value;'>";
$output .= "<option data-serviceId='0' value='Je ne suis pas certain(e)'>Je ne suis pas certain(e)</option>";
foreach ( $myposts as $post ) : setup_postdata($post);
	$title = get_the_title();
	$service = $post->post_name;
	$output .= "<option data-service='$service' value='$title'>$title</option>";
...
//wpcf7_add_shortcode('postdropdown2', 'createbox2', true);
function createbox2(){
global $post;
$args = array(  'post_type' => 'equipe', 
                ...
                'posts_per_page' => -1 );
$myposts = get_posts( $args );
$output = "<select name='select_equipe' id='select_equipe' onchange='document.getElementById(\"select_equipe\").value=this.value;'>";
$output .= "<option data-service='0' data-serviceAdd='' data-pro='0' value='Aucune préférence'>Aucune préférence</option>";
foreach ( $myposts as $post ) : setup_postdata($post);
	$title = get_the_title();
	$pro = $post->post_name;
	$service = get_field('domaine_personne')->post_name;
	if( get_field('domaineAdd_personne') ):
	$serviceAdd = get_field('domaineAdd_personne')->post_name;
	else:
	$serviceAdd = '';
	endif;
	$output .= "<option data-service='$service' data-serviceAdd='$serviceAdd' data-pro='$pro' value='$title'>$title</option>";
